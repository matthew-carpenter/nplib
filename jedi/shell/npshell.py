...
def execute(cmd_tokens):
    with open(jedi.shell.HISTORY_PATH, 'a') as history_file:
    ...
    return jedi.shell.SHELL_STATUS_RUN
...
def display_cmd_prompt():
    ...
    # Get base directory (last part of the current working directory path)
    cwd = os.getcwd()
...
def shell_loop():
    status = jedi.shell.SHELL_STATUS_RUN

    while status == jedi.shell.SHELL_STATUS_RUN:
        display_cmd_prompt()
...
def init():
    register_command("cd", jedi.shell.builtins.cd)
    register_command("exit", jedi.shell.builtins.exit)
    register_command("export", jedi.shell.builtins.export)
    register_command("getenv", jedi.shell.builtins.getenv)
    register_command("history", jedi.shell.builtins.history)
